...
#### **Bootstrap Tier (Root Account Created)**
...
├── scripts/                   # Python automation scripts (NEW)
│   ├── __init__.py
│   ├── credentials.py         # Credential management (replaces extract/get shell scripts)
│   └── cleanup.py             # AWS resource cleanup (replaces cleanup-all.sh)
├── pave_infra.tf              # Main Terraform configuration
├── Makefile                   # Task orchestration (NEW - primary interface)
...
The Makefile should provide these primary targets:
...
# Validation
make validate      # Validate terraform and Python code
...
### 1. **scripts/credentials.py**
- Use boto3 IAM client to find users by pattern matching
- Generate credential template files with AWS Console instructions
- Handle the fact that access key secrets are non-retrievable after creation
- Provide clear guidance for manual credential extraction
- Set proper file permissions (600) on generated files

### 2. **scripts/cleanup.py**
- Comprehensive AWS resource cleanup using boto3
- Handle all resource types: IAM users, roles, policies, S3 buckets
- Proper error handling and rollback capabilities
- Progress reporting with clear status messages
- Graceful handling of non-existent resources

### 3. **Common Patterns for Python Scripts**
```python
import boto3
import json
import os
...
def get_boto3_client(service):
    ...
    try:
        ...
        print(f"❌ Error connecting to AWS {service}: {e}")
        sys.exit(1)

# Resource pattern matching
def find_pave_resources(client, pattern):
    """Find resources with pave naming pattern"""
    # Implementation using boto3 list operations
