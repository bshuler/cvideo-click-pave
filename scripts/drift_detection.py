...
class DriftDetector:
    ...
    def get_role_info(self, rolename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            role_info['assume_role_policy'] = response['Role']['AssumeRolePolicyDocument']
    ...
    def compare_policies(self, aws_policies: List[Dict[str, str]], expected_policies: List[str], resource_name: str) -> Tuple[bool, List[str]]:
    ...
    def compare_inline_policies(self, aws_inline: List[str], expected_inline: List[str], resource_name: str) -> Tuple[bool, List[str]]:
        ...
        issues = []
        if missing:
            issues.append(f"Missing inline policies in {resource_name}: {', '.join(missing)}")
        if extra:
            issues.append(f"Extra inline policies in {resource_name}: {', '.join(extra)}")
    ...
    def check_developer_user(self) -> Tuple[bool, List[str]]:
        ...
        expected_attached = [
            'DeveloperExtendedPolicy',
            'AmazonEC2ReadOnlyAccess'
        ]
        expected_inline = ['DeveloperComprehensivePolicy']
        ...
        if user_info['groups']:
            issues.append(f"developer-user has unexpected groups: {', '.join(user_info['groups'])}")
    ...
    def check_admin_user(self) -> Tuple[bool, List[str]]:
        ...
        # Check groups (should be empty)
        if user_info['groups']:
            issues.append(f"admin-user has unexpected groups: {', '.join(user_info['groups'])}")
    ...
    def run_full_drift_detection(self) -> bool:
        ...
        if all_checks_passed:
            print_success("✅ NO DRIFT DETECTED - All AWS resources match Terraform configuration")
