...
def find_pave_users(iam_client) -> list:
    ...
    try:
        ...
        for user in response['Users']:
            ...
            if (username == 'admin-user' or 
                username == 'developer-user' or
                'admin-user-' in username or 
                'developer-user-' in username):
...
def find_pave_roles(iam_client) -> list:
    ...
    try:
        ...
        for role in response['Roles']:
            ...
            # Match exact names and patterns from all deployments
            if (role_name == 'CICDDeploymentRole' or
                role_name == 'DeveloperRole' or
                'CICDDeploymentRole-' in role_name or 
                'DeveloperRole-' in role_name):
...
def find_pave_policies(iam_client) -> list:
    ...
    try:
        ...
        for policy in response['Policies']:
            ...
            # Match exact names and patterns from all deployments
            if (policy_name == 'CICDS3SpecificAccess' or
                policy_name == 'PaveAdminPolicy' or
                'CICDS3SpecificAccess-' in policy_name):
                pave_policies.append({
                    'name': policy_name,
                    'arn': policy['Arn']
                })
...
def find_pave_policies(iam_client) -> list:
    ...
    try:
        ...
        for policy in response['Policies']:
            policy_name = policy['PolicyName']
            if 'CICDS3SpecificAccess-' in policy_name:
                pave_policies.append({
                    'name': policy_name,
                    'arn': policy['Arn']
                })
...
def find_pave_buckets(s3_client) -> list:
    ...
    try:
        ...
        for bucket in response['Buckets']:
            ...
            # Match exact names and patterns from all deployments
            if (bucket_name == 'pave-tf-state-bucket-us-east-1' or
                'pave-tf-state-bucket-' in bucket_name):
...
def cleanup_user_access_keys(iam_client, username: str):
...
def cleanup_user_policies(iam_client, username: str):
...
def cleanup_role_policies(iam_client, role_name: str):
...
def empty_s3_bucket(s3_client, bucket_name: str):
    ...
    try:
        print_status("    📦", f"Emptying bucket contents...")
...
def cleanup_users(iam_client, users: list):
...
def cleanup_roles(iam_client, roles: list):
...
def cleanup_policies(iam_client, policies: list):
...
def cleanup_buckets(s3_client, buckets: list):
...
def main():
    ...
    if not args.skip_confirm:
        print_status("⚠️", "This will remove ALL pave infrastructure resources (destructive!)")
