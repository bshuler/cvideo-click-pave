...
def get_aws_resources():
    ...
    try:
        ...
        pave_users = [u['UserName'] for u in users_response['Users'] 
                     if (u['UserName'] == 'admin-user' or u['UserName'] == 'developer-user' or
                         'admin-user-' in u['UserName'] or 'developer-user-' in u['UserName'])]
        
        # Count roles (exact names or legacy patterns)
        roles_response = iam_client.list_roles()
        pave_roles = [r['RoleName'] for r in roles_response['Roles'] 
                     if (r['RoleName'] == 'CICDDeploymentRole' or r['RoleName'] == 'DeveloperRole' or
                         'CICDDeploymentRole-' in r['RoleName'] or 'DeveloperRole-' in r['RoleName'])]        # Count buckets
        buckets_response = s3_client.list_buckets()
        pave_buckets = [b['Name'] for b in buckets_response['Buckets']
                       if 'pave-tf-state-bucket-' in b['Name']]
...
def main():
    ...
    try:
        ...
        if 'error' in resources:
            ...
            if resources['users'] == 0 and resources['roles'] == 0 and resources['buckets'] == 0:
