...
def test_route53_permissions():
    ...
    try:
        ...
        try:
            ...
            print(f"   ✅ Success - Retrieved zone details for {response['HostedZone']['Name']}")
        except ClientError as e:
            print(f"   ❌ Failed: {e}")
            return False
        ...
        try:
            ...
            print(f"   ✅ Success - Found {len(response['ResourceRecordSets'])} DNS records")
            
            # Show existing apps.cvideo.click records if any
            apps_records = [r for r in response['ResourceRecordSets'] 
                          if 'apps.cvideo.click' in r['Name']]
            if apps_records:
                print(f"   📍 Found {len(apps_records)} existing apps.cvideo.click records")
            else:
